...
var SHARED_PROVIDER_UI_BUNDLE_PATH =
  "/luci-static/resources/ccswitch/provider-ui/ccswitch-provider-ui.js";
var SHARED_PROVIDER_UI_STYLE_PATH =
...
function createPreset(id, providerName, baseUrl, tokenField, model, options) {
  ...
  if (meta.authMode) preset.authMode = meta.authMode;
...
var APP_OPTIONS = [
  {
    ...
    manageDescription: _(
      "Manage saved Claude-compatible providers for this router. Service settings, outbound proxy routing, and restart actions stay in LuCI.",
    ),
    baseUrlDescription: _("Claude-compatible API endpoint."),
    baseUrlPlaceholder: "https://api.anthropic.com",
    tokenDescription: _(
      "Choose which Anthropic token env key this provider should use.",
    ),
    tokenLabel: _("Token"),
    tokenFieldChoices: [DEFAULT_TOKEN_FIELD, ALT_TOKEN_FIELD],
    modelDescription: _(
      "Optional. Leave blank to avoid a forced model override.",
    ),
    modelPlaceholder: _("Optional model override"),
    summaryRunning: _(
      "The active Claude provider is ready to route traffic with the current outbound proxy settings.",
    ),
    summaryInactive: _(
      "Save a Claude-compatible provider below, then activate it when you are ready to route Claude traffic.",
    ),
    newProviderExample: _("Example: Claude Provider"),
    editorNameDescription: _(
      "Display name for this Claude-compatible provider.",
    ),
  ...
  },
  {
    ...
    manageDescription: _(
      "Manage saved Codex / OpenAI Responses providers for this router. Service settings, outbound proxy routing, and restart actions stay in LuCI.",
    ),
    baseUrlDescription: _(
      "OpenAI-compatible Responses endpoint for Codex traffic.",
    ),
    ...
    modelDescription: _(
      "Optional. Leave blank to keep the current or default Codex model.",
    ),
    modelPlaceholder: "gpt-5.4",
    summaryRunning: _(
      "The active Codex provider is ready to route traffic with the current outbound proxy settings.",
    ),
    summaryInactive: _(
      "Save a Codex provider below, then activate it when you are ready to route Codex traffic.",
    ),
  ...
  },
  {
    ...
    manageDescription: _(
      "Manage saved Gemini-compatible providers for this router. Service settings, outbound proxy routing, and restart actions stay in LuCI.",
    ),
    baseUrlDescription: _("Gemini-compatible API endpoint."),
    baseUrlPlaceholder: "https://generativelanguage.googleapis.com/v1beta",
    tokenDescription: _(
      "Gemini providers use GEMINI_API_KEY. This can also hold OAuth access-token JSON when needed.",
    ),
    tokenLabel: _("Credential"),
    tokenFieldChoices: [GEMINI_TOKEN_FIELD],
    modelDescription: _(
      "Optional. Leave blank to keep the current or default Gemini model.",
    ),
    modelPlaceholder: "gemini-3.1-pro",
    summaryRunning: _(
      "The active Gemini provider is ready to route traffic with the current outbound proxy settings.",
    ),
    summaryInactive: _(
      "Save a Gemini provider below, then activate it when you are ready to route Gemini traffic.",
    ),
...
function readDaemonAdminJson(response) {
  if (!response || typeof response.json !== "function")
    return Promise.reject(
      createDaemonAdminUnavailableError(
        "OpenWrt daemon admin API returned an invalid response object.",
      ),
    );
  ...
  if (response.ok !== true)
    return Promise.reject(
      createDaemonAdminUnavailableError(
        "OpenWrt daemon admin API responded with HTTP " + response.status + ".",
      ),
    );

  return response.json().then(function (payload) {
    if (!payload || typeof payload !== "object")
      throw createDaemonAdminUnavailableError(
        "OpenWrt daemon admin API returned invalid JSON.",
      );
...
function callDaemonAdminJson(path, options) {
  ...
  if (!baseUrl)
    return Promise.reject(
      createDaemonAdminUnavailableError(
        "OpenWrt daemon admin API disabled for this page origin.",
      ),
    );

  if (typeof fetch !== "function")
    return Promise.reject(
      createDaemonAdminUnavailableError("Browser fetch API is unavailable."),
    );

  if (request.headers && typeof request.headers === "object") {
    for (key in request.headers) headers[key] = request.headers[key];
  }
  ...
  return fetch(baseUrl + path, {
    ...
    .catch(function (error) {
      if (isDaemonAdminUnavailableError(error)) throw error;
...
function daemonAdminOrFallback(apiCall, fallbackCall) {
  return Promise.resolve()
    .then(apiCall)
    .catch(function (error) {
      if (isDaemonAdminUnavailableError(error)) return fallbackCall();
...
function callOpenWrtUsageSummary(appId) {
  return daemonAdminOrFallback(
    function () {
      return callDaemonAdminJson(
        "/apps/" + encodeURIComponent(appId) + "/usage-summary",
      );
    },
    function () {
...
function callOpenWrtProviderStats(appId) {
  return daemonAdminOrFallback(
    function () {
      return callDaemonAdminJson(
        "/apps/" + encodeURIComponent(appId) + "/provider-stats",
      );
    },
    function () {
...
function callOpenWrtRecentActivity(appId) {
  return daemonAdminOrFallback(
    function () {
      return callDaemonAdminJson(
        "/apps/" + encodeURIComponent(appId) + "/recent-activity",
      );
    },
    function () {
...
function buildRequestLogsQuery(page, pageSize, providerId) {
  ...
  if (page != null) query.push("page=" + encodeURIComponent(String(page)));
  if (pageSize != null)
    ...
    function () {
      return callDaemonAdminJson(
        "/apps/" +
          ...
          "/request-logs" +
          buildRequestLogsQuery(normalizedPage, normalizedPageSize, providerId),
      );
    },
    function () {
...
function callOpenWrtRequestDetail(appId, requestId) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(callGetRequestDetail(appId, requestId), {
        ok: false,
      });
    },
  );
...
function callOpenWrtListProviders(appId) {
  return daemonAdminOrFallback(
    function () {
      return callDaemonAdminJson(
        "/apps/" + encodeURIComponent(appId) + "/providers",
      );
    },
    function () {
...
function callOpenWrtGetActiveProvider(appId) {
  return daemonAdminOrFallback(
    function () {
      return callDaemonAdminJson(
        "/apps/" + encodeURIComponent(appId) + "/providers/active",
      );
    },
    function () {
...
function callOpenWrtGetProviderFailover(appId, providerId) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(callGetProviderFailover(appId, providerId), {
        ok: false,
      });
    },
  );
...
function callOpenWrtGetAvailableFailoverProviders(appId) {
  return daemonAdminOrFallback(
    function () {
      return callDaemonAdminJson(
        "/apps/" + encodeURIComponent(appId) + "/failover/providers/available",
      );
    },
    function () {
      return L.resolveDefault(callGetAvailableFailoverProviders(appId), {
        ok: false,
      });
    },
  );
...
function callOpenWrtCreateProvider(appId, providerPayload) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(callUpsertProvider(appId, providerPayload), {
        ok: false,
      });
    },
  );
...
function callOpenWrtUpdateProvider(appId, providerId, providerPayload) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(
        callUpsertProviderByProviderId(appId, providerId, providerPayload),
        { ok: false },
      );
    },
  );
...
function callOpenWrtSaveActiveProvider(appId, providerPayload) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(
        callUpsertActiveProvider(appId, providerPayload),
        { ok: false },
      );
    },
  );
...
function callOpenWrtDeleteProvider(appId, providerId) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(
        callDeleteProviderByProviderId(appId, providerId),
        { ok: false },
      );
    },
  );
...
function callOpenWrtActivateProvider(appId, providerId) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(
        callActivateProviderByProviderId(appId, providerId),
        { ok: false },
      );
    },
  );
...
function callOpenWrtUploadCodexAuth(appId, providerId, authJsonText) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(
        callUploadCodexAuth(appId, providerId, authJsonText),
        { ok: false },
      );
    },
  );
...
function callOpenWrtUploadClaudeAuth(appId, providerId, authJsonText) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(
        callUploadClaudeAuth(appId, providerId, authJsonText),
        { ok: false },
      );
    },
  );
...
function callOpenWrtRemoveCodexAuth(appId, providerId) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(callRemoveCodexAuth(appId, providerId), {
        ok: false,
      });
    },
  );
...
function callOpenWrtRemoveClaudeAuth(appId, providerId) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(callRemoveClaudeAuth(appId, providerId), {
        ok: false,
      });
    },
  );
...
function callOpenWrtAddToFailoverQueue(appId, providerId) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(callAddToFailoverQueue(appId, providerId), {
        ok: false,
      });
    },
  );
...
function callOpenWrtRemoveFromFailoverQueue(appId, providerId) {
  return daemonAdminOrFallback(
    ...
    function () {
      return L.resolveDefault(callRemoveFromFailoverQueue(appId, providerId), {
        ok: false,
      });
    },
  );
...
function callOpenWrtReorderFailoverQueue(appId, providerIds) {
  return daemonAdminOrFallback(
    function () {
      return callDaemonAdminJson(
        "/apps/" + encodeURIComponent(appId) + "/failover/queue",
        {
          method: "PUT",
          body: {
    ...
    function () {
      return L.resolveDefault(callReorderFailoverQueue(appId, providerIds), {
        ok: false,
      });
    },
  );
...
function callOpenWrtReorderProviders(appId, providerIds) {
  return daemonAdminOrFallback(
    function () {
      return callDaemonAdminJson(
        "/apps/" + encodeURIComponent(appId) + "/providers/order",
        {
          method: "PUT",
          body: {
            providerIds: providerIds,
    ...
    function () {
      return L.resolveDefault(callReorderProviders(appId, providerIds), {
        ok: false,
      });
    },
  );
...
function callOpenWrtSetAutoFailoverEnabled(appId, enabled) {
  return daemonAdminOrFallback(
    function () {
      return callDaemonAdminJson(
        "/apps/" + encodeURIComponent(appId) + "/failover/auto-enabled",
        {
          method: "PUT",
          body: {
            enabled: enabled,
    ...
    function () {
      return L.resolveDefault(callSetAutoFailoverEnabled(appId, enabled), {
        ok: false,
      });
    },
  );
...
function callOpenWrtSetMaxRetries(appId, value) {
  return daemonAdminOrFallback(
    function () {
      return callDaemonAdminJson(
        "/apps/" + encodeURIComponent(appId) + "/failover/max-retries",
        {
          method: "PUT",
          body: {
            value: value,
...
return view.extend({
  ...
  isSupportedApp: function (appId) {
    ...
    for (i = 0; i < APP_OPTIONS.length; i++) {
      if (APP_OPTIONS[i].id === appId) return true;
    }
  ...
  getAppMeta: function (appId) {
    ...
    for (i = 0; i < APP_OPTIONS.length; i++) {
      if (APP_OPTIONS[i].id === appId) return APP_OPTIONS[i];
    }
  ...
  inferPresetIdFromPayload: function (appId, payload) {
    var presets = this.getPresetOptions(appId);
    var normalizedBaseUrl = (payload && payload.baseUrl ? payload.baseUrl : "")
      .trim()
      .replace(/\/+$/, "");
    var normalizedTokenField =
      payload && payload.tokenField ? payload.tokenField : "";
    var i;

    for (i = 0; i < presets.length; i++) {
      var presetBaseUrl = (presets[i].baseUrl || "").trim().replace(/\/+$/, "");
      if (
        presetBaseUrl === normalizedBaseUrl &&
        (!normalizedTokenField ||
          presets[i].tokenField === normalizedTokenField)
      )
        return presets[i].id;
    }
  ...
  updatePresetDescription: function (node, appId, presetId) {
    ...
    node.textContent = preset
      ...
      : _(
          "Use a preset to prefill the fields below. You can continue editing after selection.",
        );
  },
  ...
  applyPresetToInputs: function (appId, presetId, refs) {
    ...
    refs.tokenFieldSelect.value =
      preset.tokenField || this.getAppMeta(appId).tokenFieldChoices[0];
  ...
  parseRuntimeStatusPayload: function (response) {
    ...
    service =
      parsed && parsed.service && typeof parsed.service === "object"
        ? parsed.service
        : null;
    ...
    return {
      ...
      listenAddress:
        service && service.listenAddress ? String(service.listenAddress) : "",
      listenPort:
        service && service.listenPort != null ? String(service.listenPort) : "",
      ...
      statusSource:
        service && service.statusSource ? String(service.statusSource) : "",
    };
  },
  ...
  normalizeHostConfigSnapshot: function (response) {
    ...
    if (enabled !== true && enabled !== false) enabled = fallback.enabled;

    return {
      enabled: enabled === true,
      listenAddr:
        payload.listenAddr != null
          ? String(payload.listenAddr)
          : fallback.listenAddr,
      listenPort:
        payload.listenPort != null
          ? String(payload.listenPort)
          : fallback.listenPort,
      httpProxy:
        payload.httpProxy != null
          ? String(payload.httpProxy)
          : fallback.httpProxy,
      httpsProxy:
        payload.httpsProxy != null
          ? String(payload.httpsProxy)
          : fallback.httpsProxy,
      logLevel:
        payload.logLevel != null ? String(payload.logLevel) : fallback.logLevel,
    };
  },
  ...
  loadHostConfigSnapshot: function () {
    ...
    return L.resolveDefault(callGetHostConfig(), fallback).then(
  ...
  isStaticPrototypeValidIpv6Address: function (value) {
    ...
    if (hasIpv4Tail) {
      ...
      if (!this.isStaticPrototypeValidIpv4Address(ipv4Part)) return false;
    ...
    if (emptyIndex < 0) {
      ...
      if (hasIpv4Tail) segmentCount += 1;
      if (segmentCount !== 8) return false;
    } else {
      compressedParts = (parts[0] ? parts[0].split(":") : []).concat(
    ...
    for (i = 0; i < compressedParts.length; i++) {
      if (!compressedParts[i]) return false;
      if (compressedParts[i] === "ipv4") continue;
      if (!/^[0-9A-Fa-f]{1,4}$/.test(compressedParts[i])) return false;
    }
  ...
  isStaticPrototypeValidIpAddress: function (value) {
    ...
    return (
      this.isStaticPrototypeValidIpv4Address(value) ||
      this.isStaticPrototypeValidIpv6Address(value)
    );
  },
  ...
  normalizeStaticPrototypeHostPayload: function (payload, currentSnapshot) {
    ...
    var listenAddr =
      payload && payload.listenAddr != null
        ? String(payload.listenAddr).trim()
        : snapshot.listenAddr || "";
    var listenPort =
      payload && payload.listenPort != null
        ? String(payload.listenPort).trim()
        : snapshot.listenPort || "";
    var logLevel =
      payload && typeof payload.logLevel === "string"
    ...
    if (!allowedLogLevels[logLevel]) logLevel = snapshot.logLevel || "info";
    ...
    return {
      ...
      httpProxy:
        payload && payload.httpProxy != null
          ? String(payload.httpProxy)
          : snapshot.httpProxy || "",
      httpsProxy:
        payload && payload.httpsProxy != null
          ? String(payload.httpsProxy)
          : snapshot.httpsProxy || "",
      logLevel: logLevel,
    };
  ...
  staticPrototypeHostConfigRequiresRestart: function (current, next) {
    ...
    return (
      String(before.listenAddr || "") !== String(after.listenAddr || "") ||
  ...
  saveStaticPrototypeHostConfig: function (payload) {
    return this.loadHostConfigSnapshot().then(
      L.bind(function (current) {
        var next = this.normalizeStaticPrototypeHostPayload(payload, current);
        var restartRequired = this.staticPrototypeHostConfigRequiresRestart(
          current,
          next,
        );
        var host = {
        ...
        return Promise.resolve(callSetHostConfig(host)).then(
          L.bind(function (response) {
            if (!response || response.ok === false)
              throw new Error(
                this.rpcFailureMessage(response) ||
                  _("Failed to save host settings."),
              );
  ...
  loadStaticPrototypeHostBindingsAfterRestart: function () {
    ...
    var tryLoad = function () {
      return self
        .loadStaticPrototypeHostBindings()
        .then(function (hostBindings) {
          if (
            hostBindings &&
            hostBindings.status === "running" &&
            hostBindings.health !== "unknown"
          )
            return hostBindings;

          attempts += 1;
          if (attempts >= maxAttempts) return hostBindings;
  ...
  getStaticPrototypeBindings: function (data) {
    var hostConfigResponse =
      data && data[0] && typeof data[0] === "object" ? data[0] : null;
    ...
    var hostConfig = hostConfigResponse
      ? this.setHostConfigSnapshot(hostConfigResponse)
      : this.getHostConfigSnapshot();
    var isRunning = this.parseServiceState(serviceStatus);
    var proxyEnabled =
      isRunning && runtime.statusSource
  ...
  normalizeNativeHostState: function (bindings) {
    ...
    return {
      app: this.isSupportedApp(payload.app)
        ? payload.app
        : this.getSelectedApp(),
  ...
  normalizeUsageSummary: function (response) {
    ...
    return {
      totalRequests:
        ...
        isFinite(payload.totalRequests)
          ? payload.totalRequests
          : 0,
      totalCost: payload.totalCost != null ? String(payload.totalCost) : "0",
      totalInputTokens:
        typeof payload.totalInputTokens === "number" &&
        isFinite(payload.totalInputTokens)
          ? payload.totalInputTokens
          : 0,
      totalOutputTokens:
        typeof payload.totalOutputTokens === "number" &&
        isFinite(payload.totalOutputTokens)
          ? payload.totalOutputTokens
          : 0,
      totalCacheCreationTokens:
        typeof payload.totalCacheCreationTokens === "number" &&
        isFinite(payload.totalCacheCreationTokens)
          ? payload.totalCacheCreationTokens
          : 0,
      totalCacheReadTokens:
        typeof payload.totalCacheReadTokens === "number" &&
        isFinite(payload.totalCacheReadTokens)
          ? payload.totalCacheReadTokens
          : 0,
      successRate:
        typeof payload.successRate === "number" && isFinite(payload.successRate)
          ? payload.successRate
          : 0,
    };
  },
  ...
  normalizeProviderStats: function (response) {
    var payload = response && typeof response === "object" ? response : {};
    var providers = Array.isArray(payload.providers)
      ? payload.providers
    ...
    return providers.map(function (provider) {
      ...
      return {
        ...
        providerName:
          item.providerName != null ? String(item.providerName) : "",
        requestCount:
          typeof item.requestCount === "number" && isFinite(item.requestCount)
            ? item.requestCount
            : 0,
        totalTokens:
          typeof item.totalTokens === "number" && isFinite(item.totalTokens)
            ? item.totalTokens
            : 0,
        totalCost: item.totalCost != null ? String(item.totalCost) : "0",
        successRate:
          typeof item.successRate === "number" && isFinite(item.successRate)
            ? item.successRate
            : 0,
        avgLatencyMs:
          typeof item.avgLatencyMs === "number" && isFinite(item.avgLatencyMs)
            ? item.avgLatencyMs
            : 0,
      };
    });
  ...
  normalizeRecentActivity: function (response) {
    ...
    if (
      !entries.length &&
      typeof payload.recentActivityJson === "string" &&
      payload.recentActivityJson
    ) {
      fallbackPayload = this.parseJsonString(payload.recentActivityJson);

      if (fallbackPayload && typeof fallbackPayload === "object")
        entries = Array.isArray(fallbackPayload.entries)
    ...
    return entries.map(function (entry) {
      ...
      return {
        ...
        totalCost: item.totalCost != null ? String(item.totalCost) : "0",
        statusCode:
          typeof item.statusCode === "number" && isFinite(item.statusCode)
            ? item.statusCode
            : 0,
        latencyMs:
          typeof item.latencyMs === "number" && isFinite(item.latencyMs)
            ...
            : 0,
        createdAt:
          typeof item.createdAt === "number" && isFinite(item.createdAt)
            ? item.createdAt
            : 0,
      };
    });
    ...
    return {
      data: entries.map(function (entry) {
        ...
        return {
          ...
          model: item.model != null ? String(item.model) : "",
          requestModel:
            item.requestModel != null ? String(item.requestModel) : "",
          costMultiplier:
            item.costMultiplier != null ? String(item.costMultiplier) : "1",
          inputTokens:
            typeof item.inputTokens === "number" && isFinite(item.inputTokens)
              ? item.inputTokens
              : 0,
          outputTokens:
            typeof item.outputTokens === "number" && isFinite(item.outputTokens)
              ? item.outputTokens
              : 0,
          cacheReadTokens:
            typeof item.cacheReadTokens === "number" &&
            isFinite(item.cacheReadTokens)
              ? item.cacheReadTokens
              : 0,
          cacheCreationTokens:
            typeof item.cacheCreationTokens === "number" &&
            isFinite(item.cacheCreationTokens)
              ? item.cacheCreationTokens
              : 0,
          inputCostUsd:
            item.inputCostUsd != null ? String(item.inputCostUsd) : "0",
          outputCostUsd:
            item.outputCostUsd != null ? String(item.outputCostUsd) : "0",
          cacheReadCostUsd:
            item.cacheReadCostUsd != null ? String(item.cacheReadCostUsd) : "0",
          cacheCreationCostUsd:
            item.cacheCreationCostUsd != null
              ? String(item.cacheCreationCostUsd)
              : "0",
          totalCostUsd:
            item.totalCostUsd != null ? String(item.totalCostUsd) : "0",
          ...
          latencyMs:
            typeof item.latencyMs === "number" && isFinite(item.latencyMs)
              ...
              : 0,
          firstTokenMs:
            typeof item.firstTokenMs === "number" && isFinite(item.firstTokenMs)
              ? item.firstTokenMs
              : null,
          durationMs:
            typeof item.durationMs === "number" && isFinite(item.durationMs)
              ? item.durationMs
              : null,
          statusCode:
            typeof item.statusCode === "number" && isFinite(item.statusCode)
              ...
              : 0,
          errorMessage:
            item.errorMessage != null ? String(item.errorMessage) : null,
          createdAt:
            typeof item.createdAt === "number" && isFinite(item.createdAt)
              ? item.createdAt
              : 0,
      ...
      total:
        typeof payload.total === "number" && isFinite(payload.total)
          ? payload.total
          : 0,
      page:
        typeof payload.page === "number" && isFinite(payload.page)
          ? payload.page
          : 0,
      pageSize:
        typeof payload.pageSize === "number" && isFinite(payload.pageSize)
          ? payload.pageSize
          : 0,
    };
  },
  ...
  loadNativeUsageSummary: function (appId) {
    var selectedApp = this.isSupportedApp(appId)
      ? appId
      : this.getSelectedApp();

    return L.resolveDefault(callOpenWrtUsageSummary(selectedApp), {
      ok: false,
    }).then(
      L.bind(function (response) {
        if (!this.isRpcSuccess(response))
          throw new Error(
            this.rpcFailureMessage(response) ||
              _("Failed to load usage summary."),
          );
  ...
  loadNativeProviderStats: function (appId) {
    ...
    return L.resolveDefault(callOpenWrtProviderStats(selectedApp), {
      ok: false,
    }).then(
      L.bind(function (response) {
        if (!this.isRpcSuccess(response))
          throw new Error(
            this.rpcFailureMessage(response) ||
              _("Failed to load provider stats."),
          );
  ...
  loadNativeRecentActivity: function (appId) {
    ...
    return L.resolveDefault(callOpenWrtRecentActivity(selectedApp), {
      ok: false,
    }).then(
      L.bind(function (response) {
        if (!this.isRpcSuccess(response))
          throw new Error(
            this.rpcFailureMessage(response) ||
              _("Failed to load recent activity."),
          );
  ...
  loadNativeQuota: function () {
    return L.resolveDefault(callOpenWrtQuota(), { ok: false }).then(
      L.bind(function (response) {
        if (
          !response ||
          ...
          throw new Error(
            this.rpcFailureMessage(response) || _("Failed to load quota."),
          );
  ...
  loadNativeRequestDetail: function (appId, requestId) {
    ...
    return L.resolveDefault(callOpenWrtRequestDetail(selectedApp, requestId), {
      ok: false,
    }).then(
      L.bind(function (response) {
        if (!this.isRpcSuccess(response))
          throw new Error(
            this.rpcFailureMessage(response) ||
              _("Failed to load request detail."),
          );
  ...
  refreshNativeHostState: function (uiState) {
    return this.loadStaticPrototypeHostBindings().then(
      L.bind(function (bindings) {
  ...
  saveNativeHostConfig: function (uiState, payload) {
    ...
    return this.saveStaticPrototypeHostConfig(payload)
      .then(
        L.bind(function (saveResult) {
          return this.loadStaticPrototypeHostBindings().then(
            L.bind(function (bindings) {
              var hostState = this.setNativeHostState(uiState, bindings);
              var restartRequired = !!(
                saveResult && saveResult.restartRequired
              );
      ...
      .catch(
        L.bind(function (err) {
          this.setMessage(
            uiState,
            "error",
            this.rpcFailureMessage(err) || _("Failed to save host settings."),
          );
          this.notifyShellListeners(uiState);
          throw err;
        }, this),
      ...
      .then(
        L.bind(function (result) {
          if (!this.isRpcSuccess(result))
            throw new Error(
              this.rpcError(result) || _("Failed to restart service."),
            );

          return this.loadStaticPrototypeHostBindingsAfterRestart().then(
            L.bind(function (bindings) {
      ...
      .catch(
        L.bind(function (err) {
          this.setMessage(
            uiState,
            "error",
            this.rpcFailureMessage(err) || _("Failed to restart service."),
          );
          this.notifyShellListeners(uiState);
          throw err;
        }, this),
  ...
  loadStaticPrototypeProviderState: function (appId) {
    return this.loadProviderState(appId).then(
      L.bind(function (providerState) {
        return this.loadStaticPrototypeFailoverState(appId, providerState).then(
          L.bind(function (failoverByProviderId) {
            var providers = Array.isArray(providerState.providers)
              ? providerState.providers
              : [];
            var enrichedProviders = providers.map(function (provider) {
              var providerId =
                provider && provider.providerId ? provider.providerId : null;
              var failoverState =
                providerId && failoverByProviderId[providerId]
  ...
  loadStaticPrototypeFailoverState: function (appId, providerState) {
    var self = this;
    var providers =
      providerState && Array.isArray(providerState.providers)
        ? providerState.providers
        : [];
    var requests = [];
    ...
    providers.forEach(function (provider) {
      ...
      requests.push(
        L.resolveDefault(
          callOpenWrtGetProviderFailover(appId, providerId),
          null,
        ).then(function (response) {
          return {
            providerId: providerId,
            failover: self.parseStaticPrototypeFailoverState(
              response,
              providerId,
            ),
          };
  ...
  buildStaticPrototypeWorkspaceData: function (data) {
    var providerStates =
      data && data[3] && typeof data[3] === "object" ? data[3] : {};
    var payload = {
      apps: {},
    };

    APP_OPTIONS.forEach(function (appMeta) {
      var state = providerStates[appMeta.id] || {
        providers: [],
  ...
  parseProviderState: function (providerResponse, appId) {
    ...
    if (
      ...
      providerResponse.provider_json
    )
    ...
    if (!parsed) return this.emptyProviderView(appId);
  ...
  parseStaticPrototypeFailoverState: function (response, fallbackProviderId) {
    ...
    queue = Array.isArray(parsed.failoverQueue)
      ? parsed.failoverQueue
          .map(
            L.bind(function (entry) {
      ...
      : [];
    providerId =
      this.getStaticPrototypeStringValue(parsed, [
        "providerId",
        "provider_id",
      ]) || fallbackProviderId;
    ...
    return {
      providerId: providerId,
      proxyEnabled: this.getStaticPrototypeBooleanValue(parsed, [
        "proxyEnabled",
        "proxy_enabled",
      ]),
      autoFailoverEnabled: this.getStaticPrototypeBooleanValue(parsed, [
        "autoFailoverEnabled",
        "auto_failover_enabled",
      ]),
      maxRetries: this.getStaticPrototypeNumberValue(parsed, [
        "maxRetries",
        "max_retries",
      ]),
      activeProviderId: this.getStaticPrototypeOptionalStringValue(parsed, [
        "activeProviderId",
        "active_provider_id",
      ]),
      inFailoverQueue: this.getStaticPrototypeBooleanValue(parsed, [
        "inFailoverQueue",
        "in_failover_queue",
      ]),
      queuePosition: this.getStaticPrototypeOptionalNumberValue(parsed, [
        "queuePosition",
        "queue_position",
      ]),
      sortIndex: this.getStaticPrototypeOptionalNumberValue(parsed, [
        "sortIndex",
        "sort_index",
      ]),
      providerHealth: this.parseStaticPrototypeProviderHealth(
        parsed.providerHealth,
        providerId,
      ),
      failoverQueueDepth:
        this.getStaticPrototypeNumberValue(parsed, [
          "failoverQueueDepth",
  ...
  parseStaticPrototypeFailoverQueueEntry: function (entry) {
    ...
    providerId = this.getStaticPrototypeStringValue(entry, [
      "providerId",
      "provider_id",
    ]);
    ...
    return {
      providerId: providerId,
      providerName: this.getStaticPrototypeStringValue(entry, [
        "providerName",
        "provider_name",
      ]),
      sortIndex: this.getStaticPrototypeOptionalNumberValue(entry, [
        "sortIndex",
        "sort_index",
      ]),
  ...
  parseStaticPrototypeProviderHealth: function (health, fallbackProviderId) {
    ...
    return {
      providerId:
        this.getStaticPrototypeStringValue(health, [
          "providerId",
          "provider_id",
        ]) || fallbackProviderId,
      ...
      consecutiveFailures: this.getStaticPrototypeNumberValue(health, [
        "consecutiveFailures",
        "consecutive_failures",
      ]),
      lastSuccessAt: this.getStaticPrototypeOptionalStringValue(health, [
        "lastSuccessAt",
        "last_success_at",
      ]),
      lastFailureAt: this.getStaticPrototypeOptionalStringValue(health, [
        "lastFailureAt",
        "last_failure_at",
      ]),
      lastError: this.getStaticPrototypeOptionalStringValue(health, [
        "lastError",
        "last_error",
      ]),
      updatedAt: this.getStaticPrototypeOptionalStringValue(health, [
        "updatedAt",
        "updated_at",
      ]),
    };
  ...
  extractPhase2ListPayload: function (response) {
    ...
    for (i = 0; i < directKeys.length; i++) {
      if (response[directKeys[i]] != null) return response;
    }

    return null;
  },

  normalizeProviderView: function (
    provider,
    fallbackId,
    activeProviderId,
    appId,
    ...
    if (!provider || typeof provider !== "object") return normalized;
    ...
    normalized.tokenField =
      provider.tokenField ||
      provider.token_field ||
      appMeta.tokenFieldChoices[0];
    normalized.tokenConfigured = !!(
      provider.tokenConfigured || provider.token_configured
    );
    normalized.tokenMasked =
      provider.tokenMasked || provider.token_masked || "";
  ...
  extractRawProviders: function (payload) {
    ...
    if (payload.providers != null) return payload.providers;

    if (payload.items != null) return payload.items;

    if (payload.savedProviders != null) return payload.savedProviders;

    if (payload.providerMap != null) return payload.providerMap;
  ...
  matchProviderHint: function (providers, providerHint) {
    ...
    if (!providerHint || !providerHint.configured) return null;

    for (i = 0; i < providers.length; i++) {
      if (
        providerHint.providerId &&
        providers[i].providerId === providerHint.providerId
      )
    ...
    for (i = 0; i < providers.length; i++) {
      if (
        providers[i].name === providerHint.name &&
  ...
  buildProviderState: function (
    ...
    for (i = 0; i < providers.length; i++) {
      providers[i].active = !!(
        activeProviderId && providers[i].providerId === activeProviderId
      );
      if (providers[i].active) activeProvider = providers[i];
    }
  ...
  parsePhase2ProviderState: function (listResponse, activeHint, appId) {
    ...
    activeProviderId =
      ...
      (listResponse &&
        (listResponse.activeProviderId || listResponse.active_provider_id)) ||
      null;

    providers = this.normalizeProviderList(
      this.extractRawProviders(payload),
      activeProviderId,
      appId,
    );
  ...
  loadProviderState: function (appId) {
    return Promise.all([
      ...
      L.bind(function (results) {
        ...
        var phase2State = this.parsePhase2ProviderState(
          results[0],
          activeProvider,
          appId,
        );

        if (phase2State) return phase2State;
    ...
    if (uiState.editorMode === "edit")
      return (
        this.findProviderById(
          uiState.providerState.providers,
          uiState.editProviderId,
        ) || this.emptyProviderView(uiState.selectedApp)
      );
  ...
  createUiState: function (isRunning, providerStateOrSelectedApp, selectedApp) {
    ...
    if (
      ...
      resolvedSelectedApp = this.isSupportedApp(providerStateOrSelectedApp)
        ? providerStateOrSelectedApp
        : this.getSelectedApp();
      providerState = this.buildLegacyProviderState(
        this.emptyProviderView(resolvedSelectedApp),
        resolvedSelectedApp,
      );
    }

    resolvedSelectedApp = this.isSupportedApp(resolvedSelectedApp)
      ? resolvedSelectedApp
      : this.getSelectedApp();
    activeProvider =
    ...
    return {
      ...
      editorMode: providerState.phase2Available
        ? "new"
        : activeProvider.configured
          ? "legacy"
          : "new",
      editProviderId: providerState.phase2Available
        ? null
  ...
  setEditorModeNew: function (uiState) {
    uiState.editorMode = uiState.providerState.phase2Available
      ? "new"
      : "legacy";
    uiState.editProviderId = uiState.providerState.phase2Available
      ? null
      : uiState.providerState.activeProviderId;
  },
  ...
  setEditorModeEdit: function (uiState, providerId) {
    uiState.editorMode = uiState.providerState.phase2Available
      ...
      : "legacy";
    uiState.editProviderId =
      providerId || uiState.providerState.activeProviderId;
  },
  ...
  ensureHostPageStyles: function () {
    ...
    if (
      ...
      document.getElementById(HOST_PAGE_STYLE_ID)
    )
      return;
  ...
  createSectionIntro: function (eyebrow, title, description) {
    ...
    if (description)
      children.push(
        E("p", { class: "ccswitch-host-section-description" }, [description]),
      );
  ...
  createInlineStateNotice: function (kind, title, message, detail) {
    ...
    if (title) children.push(E("strong", {}, [title]));

    if (message) children.push(E("div", {}, [message]));

    if (detail) children.push(E("div", {}, [detail]));
  ...
  createHostPageHero: function () {
    ...
    return E("section", { class: "ccswitch-host-surface" }, [
      this.createSectionIntro(
        ...
        _(
          "Keep truthful router-backed service settings, service status, and restart actions in LuCI. The provider workspace is isolated below as the main product surface.",
        ),
      ),
    ]);
  },
  ...
  decorateMapElement: function (mapEl) {
    ...
    children.forEach(function (child) {
      ...
      if (
        child.tagName === "H2" ||
        (child.classList && child.classList.contains("cbi-map-descr"))
      ) {
    ...
    if (grid.childNodes.length) mapEl.insertBefore(grid, actionsNode || null);
    if (actionsNode) self.appendClass(actionsNode, "ccswitch-host-actions");
  ...
  renderValue: function (title, control, description) {
    ...
    else if (description) fieldChildren.push(description);
  ...
  invokeRpcCandidates: async function (candidates, missingMessage) {
    ...
    for (i = 0; i < candidates.length; i++) {
      try {
        ...
        if (this.isRpcSuccess(result)) return result;

        if (
          candidates[i].compatibilityFallback &&
          this.isCompatibilityRpcFailure(result)
        ) {
        ...
        if (
          candidates[i].compatibilityFallback &&
          this.isCompatibilityRpcFailure(err)
        ) {
    ...
    throw new Error(
      this.rpcFailureMessage(lastCompatibilityFailure) || missingMessage,
    );
  },

  invokePhase2Upsert: function (appId, providerId, providerPayload) {
    var missingMessage = _(
    ...
    if (providerId) {
      return this.invokeRpcCandidates(
        [
          {
            call: function () {
              return callOpenWrtUpdateProvider(
                appId,
                providerId,
                providerPayload,
              );
    ...
    return this.invokeRpcCandidates(
      [
        {
          call: function () {
            return callOpenWrtCreateProvider(appId, providerPayload);
          },
          compatibilityFallback: true,
        },
      ],
  ...
  createNativePageShellBridge: function (uiState) {
    ...
    return {
      ...
      setSelectedApp: function (appId) {
        if (!self.isSupportedApp(appId)) return uiState.selectedApp;

        uiState.selectedApp = appId;
        self.saveSelectedApp(appId);
        if (uiState.hostState) uiState.hostState.app = appId;
      ...
      getHostState: function () {
        return Object.assign(
          {},
          uiState.hostState ||
            self.normalizeNativeHostState({ app: uiState.selectedApp }),
        );
      },
      getMessage: function () {
      ...
      subscribe: function (listener) {
        ...
        return function () {
          ...
          uiState.shellListeners.forEach(function (candidate) {
            if (candidate !== listener) nextListeners.push(candidate);
          });
      ...
      getRequestLogs: async function (appId, page, pageSize, providerId) {
        return self.loadNativeRequestLogs(
      ...
      getRequestDetail: async function (appId, requestId) {
        return self.loadNativeRequestDetail(
          appId || uiState.selectedApp,
          requestId,
        );
      },
  ...
  loadSharedProviderBundle: function () {
    ...
    this._sharedProviderBundlePromise = new Promise(function (resolve, reject) {
      ...
      var finish = function () {
        ...
        if (api && typeof api.mount === "function") resolve(api);
        else
          reject(
            new Error(
              _("The shared provider bundle did not register a mount API."),
            ),
          );
      };
      ...
      script.addEventListener(
        ...
        function () {
          reject(
            new Error(
              _("The shared provider bundle could not be loaded from ") +
                self.getBundleAssetPath(),
            ),
          );
        },
        { once: true },
      ...
      if (
        existingScript &&
        existingScript.getAttribute("data-ccswitch-loaded") === "1"
      )
        finish();
    })
      .then(function (api) {
        var loadedScript = document.getElementById(
  ...
  renderNativePage: function (data) {
    ...
    var uiState = this.createUiState(
      hostBindings.status === "running",
      selectedApp,
    );
    var wrapper = E("div", {
      ...
      self
        ...
        .then(function (api) {
          ...
          if (!api || typeof api.mountPage !== "function") {
            throw new Error(
              _(
                "The shared provider bundle did not register a page-shell mount API.",
              ),
            );
          }
        ...
        .catch(function (err) {
          ...
          wrapper.appendChild(
            self.createInlineStateNotice(
              "error",
              _("Native page shell unavailable"),
              self.rpcFailureMessage(err) ||
                _(
                  "The OpenWrt-native page shell failed to load. Reinstall the package or refresh the page after updating the browser bundle.",
                ),
            ),
          );
        });
