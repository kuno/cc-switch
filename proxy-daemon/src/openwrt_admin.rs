...
use crate::proxy::providers::{
    ...
    codex_oauth_store::{
        ...
        load_codex_auth_summary_for_provider, save_codex_auth_for_provider,
        SavedAuthSummary,
    },
};
...
fn rollback_provider_upsert(
    ...
    match previous_provider {
        ...
        None => db
            ...
            .map_err(|e| anyhow!("failed to roll back {} provider create: {e}", profile.app_id)),
    }
}
...
fn upsert_provider_with_payload(
    ...
    {
        rollback_provider_upsert(db, profile, &provider.id, previous_provider.as_ref())
